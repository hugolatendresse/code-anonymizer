...
from common_paths import join_to_root_dir
...
# TODO find a csv containing frequency of each english word + remove all the ones never uses (and reremove the dirty ones + keep 4-6 letters)

class SanitizerPython:
    def __init__(self, token_mode="dictionary"):
        ...
        if token_mode == "dictionary":
            with open('short_word_list.json', 'r') as f:
...
def test_quick(query):
    print("Original:")
    print(query)
    a = SanitizerPython(token_mode="random")
    a = SanitizerPython(token_mode="dictionary")
...
if __name__ == "__main__":
    ...
    with open(join_to_root_dir('tests','test_query_long.txt'), 'r') as file:
        longquery = file.read().replace('\n', '')
